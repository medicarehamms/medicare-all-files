...
import AppointmentIcon from '../../svg/Appointment.svg';
...
class Dashboard extends React.Component {
	constructor(props) {
		...
		this.state = {
			/**
			 * @type {'home' | 'appointment' | 'history' | 'account'}
			 */
			page: 'home'
		};
	};
	...
	render() {
		return (
			...
			>
				...
				{
					this.state.page === 'home' ? <Home navigation={this.props.navigation} changePage={(page) => {
						this.setState({ page: page });
					}} /> :
						this.state.page === 'appointment' ? <Appointment navigation={this.props.navigation} changePage={(page) => {
							this.setState({ page: page });
						}} /> : this.state.page === 'history' ? <History navigation={this.props.navigation} /> : <Account navigation={this.props.navigation} />
				}
				...
				<View style={style.footer}>
					<TouchableOpacity
						style={{
							...
							opacity: this.state.page === 'home' ? 1 : 0.5
						}}
					...
					>
						...
						/>
						<Heading head={6} style={{ color: globalStyles.colors.primary }}>Home</Heading>
					</TouchableOpacity>
					<TouchableOpacity
						style={{
							...style.footerIcon,
						...
						onPress={() => this.setState({ page: 'appointment' })}
					>
						<AppointmentIcon
							height={globalStyles.rem * 2} // 20
							width={globalStyles.rem * 2} // 20
						/>
						<Heading head={6} style={{ color: globalStyles.colors.primary }}>Appointment</Heading>
					</TouchableOpacity>
					<TouchableOpacity
						style={{
							...style.footerIcon,

							opacity: this.state.page === 'history' ? 1 : 0.5
						}}

						onPress={() => this.setState({ page: 'history' })}
					>
						<HistoryIcon
							height={globalStyles.rem * 2} // 20
							width={globalStyles.rem * 2} // 20
						/>
						<Heading head={6} style={{ color: globalStyles.colors.primary }}>History</Heading>
					</TouchableOpacity>
					<TouchableOpacity
						style={{
							...style.footerIcon,
