...
class Account extends React.Component {
	...
	async getProfile() {
		...
		const doctor = JSON.parse(credentials);

		const response = await fetch(`${globals.apiURL}/api/doctors/${doctor.id}`);
		if (!response.ok) {
	...
	async componentDidMount() {
		const subscibe = async () => {
			...
			globals.ws.send(JSON.stringify({
				type: 'subscribe',
				page: `doctors/${user.id}`
			}));
	
			globals.ws.onmessage = (event) => {
				const data = JSON.parse(event.data) ?? {};
				if (data.type === 'refresh' && data.from === `doctors/${user.id}`) {
						...
						this.state.editMode ? (
								...
								>
									<Button
										...
										onPress={async () => {
											...
											const response = await fetch(`${globals.apiURL}/api/doctors/${this.state.id}`, {
												method: 'PATCH',
												headers: {
							...
							<>
									...
									this.state.profile.about && (
										<View
											style={{
												...
												borderBottomWidth: globalStyles.rem / 10, // 1
												paddingBottom: globalStyles.rem // 10
											}}
										>
											<Heading head={5.5}>About</Heading>
											<Text style={{ width: '100%', textAlign: 'center' }}>{this.state.profile.about}</Text>
										</View>
									)
								}
								...
								<Button
									...
									onPress={async () => {
										const credentials = await AsyncStorage.getItem('credentials');
										const response = await fetch(`${globals.apiURL}/api/doctors/deauthenticate`, {
											method: 'POST',
											headers: {
												'Content-Type': 'application/json',
												'Authorization': await (async () => { const credentials = await AsyncStorage.getItem('credentials'); return credentials ? `${JSON.parse(credentials).id} ${JSON.parse(credentials).sessionKey}` : '' })()
